...
map.redraw = function() {
  ...
  $div.style('left', function(d) { return xScale(d.x) + 'px'; })
...
function mousemove() {
  var p = d3.mouse(this);
  root.px = xScale.invert( p[0] );
  root.py = yScale.invert( p[1] );
  force.resume();
}

d3.select('body')
  .on('mousemove', mousemove)
  .call( d3.behavior.zoom().x( xScale ).y( yScale ).scaleExtent([1, 8]).on('zoom', map.redraw) );
